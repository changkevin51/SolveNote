...
class EditorState extends State<Editor> {
  ...
  // Math expression analysis callbacks
  void _onMathExpressionsDetected(List<MathExpression> expressions) {
    setState(() {
      _detectedExpressions = expressions;
    });

    log.info('Detected ${expressions.length} math expressions');
    for (final expression in expressions) {
      log.info(
          'Expression ${expression.id}: ${expression.strokes.length} strokes, bounds: ${expression.boundingBox.toRect()}');
    }
  }

  void _onMathExpressionsCleared() {
    setState(() {
      _detectedExpressions = [];
    });
